%% THIS SCRIPT READS AND CLEANS EACH COMPONENT SEPARATELY FOR WFG2.
...
load('separatedWFG2.mat','Comp1','Comp2','Comp3','Comp4','Comp5','Comp6')
num_comp = 5;
ref_size = 300;
%------------------------------------------%
...
interpol_size = [1000000,1000000,1000000,500000,100000];
% interpol_size = [1000,1000,1000,1000,1000];
%Other Parameters%
trimming = zeros(1,num_comp);

%--Cleaning--%
clean_method = {'long','long','long','long','long'};
threshold = [0.68,0.2,0.2,0.2,0.053];
for comp=1:num_comp
...
max_iters = 500;
[~,Z] = kmeans(Iy,ref_size-1,'Options',statset('MaxIter',max_iters));
Z(end+1,:) = Comp6;
Iy(end+1,:) = Comp6;
%------------------------------------------%
